...
const initialState = {
    ...
    ocupacion: '',
    email: '',
    posicionIVA: 'Consumidor_Final', // <-- Valor inicial
    pais: '',
    provincia: '',
    localidad: '',
    codigoPostal: '',
...
// Función auxiliar para calcular si es mayor de edad (18 años)
...
// --- ✨ REGLAS DE VALIDACIÓN EN TIEMPO REAL ACTUALIZADAS ✨ ---
// Ahora recibe todo el formData para validaciones condicionales
const validateField = (name, value, allFormData) => {
    ...
    // Validación de campos requeridos (si están vacíos)
    if (
        (name !== 'cuit' && name !== 'email' && name !== 'departamento' && name !== 'piso') &&
        value.trim() === ''
    ) {
        return null;
    }
    ...
    // 🚨 VALIDACIÓN CONDICIONAL: CUIT vs Posición IVA
    if (name === 'cuit' || name === 'posicionIVA') {
        const currentIVA = allFormData.posicionIVA;
        const currentCUIT = allFormData.cuit;

        const isRequired = currentIVA !== 'Consumidor_Final';

        // Si es requerido y está vacío, generamos el error en CUIT
        if (isRequired && currentCUIT.trim() === '') {
            // Solo ponemos el error si estamos revisando CUIT
            if (name === 'cuit') {
                errors[name] = 'El CUIT es obligatorio para esta posición frente al IVA.';
                return errors[name]; // Retorno temprano para no sobreescribir con error de formato
            }
        }
    }
    ...
    switch (name) {
        ...
        case 'apellidos':
        case 'nacionalidad':
        case 'ocupacion': // Ocupación también se añade aquí, se espera texto
            if (value.trim() !== '' && !onlyLettersRegex.test(value)) {
                errors[name] = `El campo solo puede contener letras y espacios.`;
            }
            break;
        ...
        case 'fechaNacimiento':
            ...
            if (value.trim() !== '' && inputDate > today) {
                errors[name] = 'La fecha de nacimiento no puede ser una fecha futura.';
            }
            else if (value.trim() !== '' && !isAdult(value)) {
                errors[name] = 'El huésped debe ser mayor de 18 años.';
            }
            break;

        case 'numeroDocumento':
        case 'telefono':
        case 'numero': // Número de la calle
            if (value.trim() !== '' && !onlyNumbersRegex.test(value)) {
                errors[name] = `El ${name === 'numero' ? 'número' : name} solo puede contener números.`;
            }
            break;

        case 'cuit':
            // Validación de formato CUIT (si ya tiene un valor)
            if (value.trim() !== '' && !onlyNumbersRegex.test(value)) {
                errors[name] = 'El CUIT solo puede contener números.';
            }
            break;

        case 'codigoPostal':
            if (value.trim() !== '' && (!onlyNumbersRegex.test(value) || !cpRegex.test(value))) {
                errors[name] = 'El código postal debe contener entre 4 y 8 números.';
            }
            break;

        case 'email':
            if (value.trim() !== '' && !emailRegex.test(value)) {
                errors[name] = 'El formato del email no es válido.';
            }
            break;
...
export default function Formulario() {
    ...
    const handleChange = (e) => {
        ...
        // 3. Validar el campo cambiado en tiempo real
        const errorMessage = validateField(name, value, updatedData);

        setErrores((prev) => {
            const newErrores = { ...prev };

            // Manejar el error del campo actual
            if (errorMessage) {
                newErrores[name] = errorMessage;
            } else {
                delete newErrores[name];
            }

            // 🚨 CRUCIAL: Si cambiamos la Posición IVA, re-validamos el CUIT para ver si ahora es obligatorio
            if (name === 'posicionIVA' || name === 'cuit') {
                const cuitError = validateField('cuit', updatedData.cuit, updatedData);
                if (cuitError) {
                    newErrores.cuit = cuitError;
                } else {
                    delete newErrores.cuit;
                }
            }

            // Limpiar error de servidor de CUIT si el usuario empieza a corregirlo
            if (name === 'cuit' && newErrores['cuitConsistente']) {
                delete newErrores['cuitConsistente'];
            }
    ...
    const handleSubmit = async (e) => {
        ...
        // 🔴 AGREGAR VALIDACIÓN DE FRONT-END ANTES DEL FETCH
        const allFormErrors = {};
        let isFormValid = true;

        // Validación de campos requeridos y formato (más estricta para el submit)
        Object.keys(initialState).forEach(key => {
            const value = formData[key];

            // Usar la función de validación que ya incluye la lógica condicional de CUIT/IVA
            const errorMessage = validateField(key, value, formData);

            // 1. Validación de formato/condicional en el submit
            if (errorMessage) {
                allFormErrors[key] = errorMessage;
                isFormValid = false;
            }

            // 2. Validación de campos requeridos (si están vacíos)
            const optionalFields = ['cuit', 'email', 'departamento', 'piso'];

            // Excluimos CUIT del chequeo genérico, ya que validateField lo manejó condicionalmente.
            if (key === 'cuit') return;

            // Chequeo genérico para campos requeridos no condicionales
            if (!optionalFields.includes(key) && value.trim() === '') {
                allFormErrors[key] = `El campo es requerido.`;
                isFormValid = false;
            }
        });

        if (!isFormValid) {
            setErrores(allFormErrors);
            setModalData({
                titulo: "Advertencia",
                descripcion: "Se encontraron errores en la carga de datos. Por favor, revise los campos marcados.",
                cancelText: "Cerrar",
                confirmText: "Aceptar",
                onCancel: () => setModalData(null),
                onConfirm: () => setModalData(null)
            });
            return; // Detener el envío si hay errores de validación de front-end
        }
        // 🔴 FIN DE VALIDACIÓN DE FRONT-END

        // ... (El resto de la lógica de fetch no necesita cambios)
