...
// Asegúrate de tener la función buscarResponsablePorCuit en tu api.js
...
const FacturacionPage = () => {
    ...
    // -----------------------------------------------------------------------
    // LÓGICA DE SELECCIÓN DE RESPONSABLE
    // -----------------------------------------------------------------------

    const seleccionarOcupante = (ocupante) => {
        setResponsableSeleccionado(ocupante);
        setEsTercero(false);
        // Si es CUIT/CUIL largo es probable que sea A, si es DNI corto es B
    ...
    const activarTercero = () => {
        setResponsableSeleccionado(null);
        setEsTercero(true);
        setTipoFactura('A'); // Por defecto empresa es A
    };

    // --- FUNCIÓN CLAVE: VALIDAR Y BUSCAR TERCERO ---
    const handleValidarTercero = async () => {
        const cuitIngresado = datosTercero.cuit.trim();

        // CASO 1: CUIT VACÍO -> PREGUNTAR SI QUIERE CREAR UNO NUEVO
        if (!cuitIngresado) {
            setActionModal({
                show: true,
                ...
                action: () => router.push('/nuevo-responsable')
            });
            return;
        }

        // CASO 2: TIENE CUIT -> BUSCAR EN BASE DE DATOS
        setValidandoCuit(true);
        try {
            const responsableEncontrado = await buscarResponsablePorCuit(cuitIngresado);

            if (responsableEncontrado) {
                // A) ENCONTRADO: Autocompletar y seleccionar
                setDatosTercero(prev => ({
                    ...prev,
                    razonSocial: responsableEncontrado.razonSocial
                }));

                setResponsableSeleccionado({
                    id: responsableEncontrado.id, // ID real de la BD
                    nombreCompleto: responsableEncontrado.razonSocial,
                    documento: responsableEncontrado.cuit,
                    tipo: 'JURIDICA'
                });

                // --- CAMBIO: Forzar Factura A ---
                setTipoFactura('A');

            } else {
                // B) NO ENCONTRADO: Preguntar si quiere crearlo
                setActionModal({
                    show: true,
                    title: "CUIT No Encontrado",
                    desc: `El CUIT ${cuitIngresado} no existe en la base de datos. ¿Desea ir al alta para registrarlo con todos sus datos (Dirección, Teléfono)?`,
                    confirmText: "Ir al Alta",
                    cancelText: "Cargar Manualmente",
                    action: () => {
                        // Guardamos el CUIT para que aparezca en el otro formulario
                        localStorage.setItem('nuevoResponsableCreado', JSON.stringify({
                            cuit: cuitIngresado,
                            razonSocial: datosTercero.razonSocial // Por si ya escribió algo
                        }));
                        router.push('/nuevo-responsable');
                    },
                    // Si cancela, permitimos que siga cargando manual (ID 999)
                    onCancelCustom: () => {
            ...
            console.error(err);
            // Si falla la API, permitimos carga manual si hay razón social
    ...
    // -----------------------------------------------------------------------
    // LÓGICA DE ÍTEMS Y TOTALES
    // -----------------------------------------------------------------------
    const toggleItem = (item) => {
        const existe = itemsSeleccionados.find(i => i.descripcion === item.descripcion);
        if (existe) {
            setItemsSeleccionados(itemsSeleccionados.filter(i => i.descripcion !== item.descripcion));
        } else {
    ...
    const calcularTotal = () => {
        return itemsSeleccionados.reduce((acc, item) => acc + item.subtotal, 0);
    };
    ...
    const handleConfirmarFactura = async () => {
        ...
        try {
            const solicitud = {
                ...
                // Enviamos datos del tercero si es ID 999 o si se seleccionó manual
            ...
            try {
                ...
                if(respuestaBack && respuestaBack.id) {
                    datosParaVisualizar.id = respuestaBack.id;
                }
            } catch (backendError) {
    ...
    return (
        <div className={styles.container}>
            ...
            {errorModal.show && (
                <ErrorModal
                    titulo={errorModal.title}
                    descripcion={errorModal.msg}
                    onClose={() => setErrorModal({ show: false, title: '', msg: '' })}
                />
            )}
            ...
            {actionModal.show && (
                <ActionModal
                    ...
                    descripcion={actionModal.desc}
                    confirmText={actionModal.confirmText || "Aceptar"}
                    cancelText={actionModal.cancelText || "Cancelar"}
                    onConfirm={() => {
                        if(actionModal.action) actionModal.action();
                        setActionModal({ ...actionModal, show: false });
                    }}
                    onCancel={() => {
                        if(actionModal.onCancelCustom) actionModal.onCancelCustom();
                        else setActionModal({ ...actionModal, show: false });
                    }}
                />
            )}
                ...
                <div className={styles.card} style={{ maxWidth: '500px' }}>
                    ...
                    <form onSubmit={handleBuscarEstadia}>
                        <div className={styles.formGroup}>
                            <label className={styles.label}>Nro. de Habitación</label>
                            <input
                                className={styles.input}
                                placeholder="Ej: 104"
                                value={busqueda.habitacion}
                                onChange={(e) => setBusqueda({...busqueda, habitacion: e.target.value})}
                                autoFocus
                            />
                        </div>
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                onChange={(e) => setBusqueda({...busqueda, horaSalida: e.target.value})}
                            />
                        </div>
            ...
            {/* --- PASO 2: INTERFAZ DE FACTURACIÓN --- */}
            {etapa === 2 && resumen && (
                <div className={styles.card}>

                    <div className={styles.header}>
                        <div>
                            <h1 className={styles.title}>Habitación {resumen.numeroHabitacion}</h1>
                            <p className={styles.subtitle}>Seleccione quién pagará la factura</p>
                        </div>
                        <div className={styles.badge}>Total: ${calcularTotal().toLocaleString()}</div>
                    </div>
                    ...
                    <div className={styles.gridTwoColumns}>
                        ...
                        <div className={styles.column}>
                            ...
                            <div className={styles.ocupantesList}>
                                {resumen.posiblesResponsables.length > 0 ? (
                                    resumen.posiblesResponsables.map((ocupante) => (
                                        <div
                                            key={ocupante.id}
                                            className={`${styles.ocupanteCard} ${responsableSeleccionado?.id === ocupante.id ? styles.ocupanteSelected : ''}`}
                                            onClick={() => seleccionarOcupante(ocupante)}
                                        >
                                            <div style={{ marginRight: '15px', color: '#6b7280' }}>
                                                <FaUser size={20} />
                                            </div>
                                            <div>
                                                <div className={styles.name}>{ocupante.nombreCompleto || "Sin Nombre"}</div>
                                                <div className={styles.details}>{ocupante.documento}</div>
                                            </div>
                                            {responsableSeleccionado?.id === ocupante.id && (
                            ...
                            {/* BOTÓN / FORMULARIO TERCERO */}
                            {!esTercero ? (
                                <button className={styles.terceroButton} onClick={activarTercero}>
                                    <FaBuilding /> Facturar a Tercero / Empresa
                                </button>
                            ) : (
                                <div className={styles.terceroForm}>
                                    {/* Título más pequeño y sin clase 'label' para evitar tamaños extraños */}
                                    <h4 style={{ marginBottom: '15px', color: '#333', fontSize: '1rem', fontWeight: 'bold' }}>
                                        Datos de Facturación (Empresa)
                                    </h4>

                                    <div className={styles.formRow}>
                                        <label className={styles.label} style={{ fontSize: '0.85rem' }}>CUIT (Dejar vacío para crear nuevo)</label>
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            <input
                                                className={styles.input}
                                                placeholder="Ingrese CUIT"
                                                value={datosTercero.cuit}
                                                onChange={(e) => setDatosTercero({...datosTercero, cuit: e.target.value})}
                                            />
                                    ...
                                    {/* CORRECCIÓN VISUAL: Label idéntico al de CUIT */}
                                    <div className={styles.formRow}>
                                        <label className={styles.label} style={{ fontSize: '0.85rem' }}>Razón Social</label>
                                        <input
                                            className={styles.input}
                                            placeholder="Nombre de la Empresa"
                                            value={datosTercero.razonSocial}
                                            onChange={(e) => setDatosTercero({...datosTercero, razonSocial: e.target.value})}
                                        />
                                    </div>
                                    ...
                                    <div className={styles.formActions}>
                                        ...
                                        <button
                                            ...
                                            style={{ marginTop: 0 }}
                                            onClick={handleValidarTercero}
                                            disabled={validandoCuit}
                                        >
                                            {validandoCuit ? 'Buscando...' : 'Validar'}
                                        </button>
                                    </div>
                                </div>
                            )}
                            ...
                            {(responsableSeleccionado) && (
                                <div style={{ marginTop: '20px', padding: '15px', background: '#f0f9ff', borderRadius: '8px' }}>
                                    ...
                                    <select
                                        ...
                                        // Si es tercero, deshabilitamos para que no puedan cambiar a B
                                        disabled={esTercero}
                                    >
                        ...
                        <div className={styles.column}>
                            ...
                            <div className={styles.tableContainer}>
                                <table className={styles.table}>
                                    ...
                                    {resumen.items.map((item, idx) => {
                                        const activo = itemsSeleccionados.some(i => i.descripcion === item.descripcion);
                                        return (
                                            <tr key={idx} style={{ opacity: activo ? 1 : 0.5 }}>
                                                <td>
                                                    <div style={{ fontWeight: '500' }}>{item.descripcion}</div>
                                                    {item.esEstadia && <span style={{ fontSize: '0.75rem', color: '#16a34a' }}>● Cargo por Estadía</span>}
                                                </td>
                            ...
                            <div className={styles.summaryBox}>
                                <div className={styles.totalRow}>
                                    ...
                                    <span className={styles.totalValue}>${calcularTotal().toLocaleString()}</span>
                                </div>
                                <div className={styles.actions}>
                                    <button className={styles.buttonSecondary} onClick={() => setEtapa(1)}>Atrás</button>
                                    <button className={styles.buttonSuccess} onClick={handleConfirmarFactura}>
                                        CONFIRMAR FACTURA
                                    </button>
                                </div>
                            </div>
